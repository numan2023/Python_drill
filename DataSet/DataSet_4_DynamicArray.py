...
for _ in range(Q):
    ...
    if cmd == 0:
        # push_back
        A.append(query[1])
        # AttributeError: 'list' object has no attribute(属性・特性) 'appned'：リストオブジェクトに対して、存在しない属性やメソッドが呼び出されたことを意味する。
    elif cmd ==1:
        # pop_back
        A.pop()
    else:
...
# appned()メソッド:リストの末尾に要素を追加する
# pop()メソッド:リストの末尾の要素を削除する
# join()メソッド:"区切り文字".join(文字列のみを要素として持つイテラブル(リストやタプル、集合などが該当))